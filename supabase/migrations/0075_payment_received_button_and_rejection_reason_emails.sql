-- =============================================================================
-- Three small email-copy fixes, bundled since they're all one-line body_html
-- corrections to default_email_templates():
--
-- 1. payment_received's button said "View Quote" — irrelevant once payment
--    is in, since the link now lands on an invoice/download view (fully
--    paid) or a remaining-balance-due view (partial payment), never a quote
--    to decide on. "View Booking Details" is accurate for both.
-- 2. job_rejected_by_supplier / job_reapproval_rejected now have a reason
--    behind every rejection (see job_rejection_log, 0074) — surface it in
--    the email itself, not just on the Dispatch job detail page.
-- 3. job_reapproval_required's copy still said "if you reject, it's simply
--    taken off your schedule, no explanation needed" — stale now that a
--    reason is required to reject.
-- =============================================================================

update email_templates
set body_html = $q$<p>Hi {{customer_name}},</p>
...
<p><a href="{{link}}" style="display:inline-block;background:#f97316;color:#ffffff;padding:12px 20px;border-radius:8px;text-decoration:none;font-weight:bold;">View Booking Details</a></p>
<p>Thanks,<br />{{brand_name}}</p>$q$
where key = 'payment_received';

update email_templates
set body_html = $q$<p><b>{{supplier_name}}</b> has declined the job for quote <b>{{quote_number}}</b>.</p>
...
where key = 'job_rejected_by_supplier';
...
update email_templates
set body_html = $q$<p>Hi {{supplier_name}},</p>
...
<p>{{changes_summary}}</p>
<p>{{payout_note}}</p>
<p>Please review and approve or reject it — if you reject, you'll be asked to give a reason.</p>
...
where key = 'job_reapproval_required';
